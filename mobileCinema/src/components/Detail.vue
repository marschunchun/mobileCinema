<template>
    <div id="detailContrainer" class="Silde-enter-active">
        <Header title="影片详情">
			...
			<div class="detail_player swiper-container" ref="detail_player">
				<ul class="swiper-wrapper">
					<li class="swiper-slide" v-for="(item,index) in detailMovie.photos" :key="index">
						<div class="img-out">
							<img :src="item|setWH('100.100')" alt="">
						</div>
					</li>
				</ul>
			</div>
...
export default {
	...
	mounted(){
		...
		this.$http.get('/api/api/detailmovie?movieId='+this.movieId).then((res)=>{
...
#detailContrainer.Silde-enter-active{animation:0.3s slideMove}
@keyframes slideMove {
...
.detail_player .swiper-slide{ width:70px;float: left; margin-right: 20px; text-align: center; font-size: 14px;}
.detail_player .swiper-slide img{ width:70px; margin-bottom: 5px;}
